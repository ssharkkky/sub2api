export default {
  promptAudit: {
    title: 'Prompt Audit',
    description: 'Review user input asynchronously or block it synchronously through Qwen3Guard, OpenAI Moderation, or Nemotron 3.5 Content Safety nodes. Full prompts are stored with events for admin review.',
    ...
    scannerDescriptions: {
      ...
      keyword: 'The prompt matched a configured keyword rule',
      hash: 'The prompt hash matched an earlier AI audit block',
    },
    runtime: {
    ...
    pool: {
      title: 'Audit pool', description: 'Enabled Qwen3Guard, OpenAI Moderation, or Nemotron nodes are tried in order. Probes run from the server network.',
      proxy: 'Proxy server', proxyHint: 'Connection tests, synchronous audits, and asynchronous audits all use this proxy. Leave unselected for a direct connection.',
      add: 'Add node', edit: 'Edit node', empty: 'No audit nodes configured.', node: 'Node', protocol: 'Protocol', openaiCompatible: 'OpenAI-compatible (Qwen3Guard)', openaiModeration: 'OpenAI Moderation', nemotronContentSafety: 'Nemotron 3.5 Content Safety', model: 'Model', limits: 'Timeout / chunk limit', credential: 'Credential and probe',
      nemotronHint: 'Supports all nine risk categories using a custom safety policy. Reasoning is disabled to reduce latency. Choose the provider alias or OpenRouter model name.',
    ...
    policy: {
      ...
      keywordBlockingMode: 'Keyword blocking strategy', keywordModeAIOnly: 'AI audit only', keywordModeAIOnlyDesc: 'Preserve the existing behavior: use configured AI audit nodes only; the keyword list is ignored.',
      keywordModeKeywordOnly: 'Keywords only', keywordModeKeywordOnlyDesc: 'Use keywords only; the keyword sync switch decides whether hits are blocked before dispatch or recorded asynchronously after dispatch.',
      keywordModeKeywordAndAI: 'Keywords + AI audit', keywordModeKeywordAndAIDesc: 'Use both mechanisms; the two synchronous blocking switches control them independently.',
      keywordModeAIOnlyNotice: 'The current strategy is “AI audit only”.', keywordModeKeywordOnlyNotice: 'The current strategy is “Keywords only”.', keywordModeKeywordAndAINotice: 'The current strategy is “Keywords + AI audit”.',
      keywordOnlyBlockingHint: 'Synchronous mode: a keyword hit is blocked immediately without AI; a miss is allowed immediately.',
      keywordOnlyAsyncHint: 'Asynchronous mode: a keyword hit is recorded as a block after the request is sent; a miss is recorded only when safe-event storage is enabled.',
      keywordAndAIBothBlockingHint: 'Synchronous mode: keyword hits are blocked immediately without AI; misses go to synchronous AI audit and blocking.',
      keywordAndAIMixedHint: 'Mixed mode: keyword hits are blocked immediately without AI; misses are allowed immediately and sent to AI for asynchronous audit only.',
      keywordAndAIOnlyAIBlockingHint: 'AI synchronous mode: keywords do not block synchronously; AI audits the request synchronously and decides whether to block it.',
      keywordAndAIAsyncHint: 'Asynchronous mode: a keyword hit is recorded as a block after the request is sent; a miss continues to AI audit.',
      blockedKeywords: 'Blocked keywords', blockedKeywordsPlaceholder: 'One keyword per line, for example:\nsensitive-term-1\nsensitive-term-2', blockedKeywordCount: '{count} keywords configured',
      blockedKeywordsLimit: 'Up to 10,000 keywords, each no longer than 200 characters; duplicates are removed automatically. This list is ignored in AI-audit-only mode.',
      preHashCheck: 'Enable pre-hash check',
      preHashCheckHint: 'After async AI audit returns Block, save the full prompt SHA-256. The same prompt is blocked before dispatch next time without another AI call.',
      flaggedHashCount: '{count} blocked hashes stored',
      flaggedHashHint: 'Only SHA-256 digests are stored, never raw prompts. Hash hits still use the current group prompt-audit fallback behavior.',
      flaggedHashPlaceholder: 'Enter a 64-character SHA-256 hash',
      deleteFlaggedHash: 'Delete hash',
      clearFlaggedHashes: 'Clear all hashes',
      clearFlaggedHashesConfirm: 'This removes every prompt-audit blocked hash. Matching prompts will need to go through async AI audit again. This cannot be undone.',
    },
    saveBar: { enabled: 'Enable prompt audit', keywordBlocking: 'Keyword synchronous blocking', aiBlocking: 'AI synchronous blocking', blockingLatestTurnOnly: 'Audit latest input and previous output only', storePass: 'Store safe events', dirty: 'Unsaved changes', synced: 'Configuration synced' },
    blockingConfirm: {
    ...
    events: {
      title: 'Audit events', description: 'Review events by identity, route, risk, hash, and time; the detail view shows the full prompt.', decision: 'Decision', risk: 'Risk level', auditType: 'Audit method', auditTypes: { ai: 'AI audit', keyword: 'Keyword blocking', hash: 'Hash blocking' }, endpoint: 'Endpoint', groupId: 'Group ID', userId: 'User ID', apiKeyId: 'API Key ID', keyword: 'Keyword',
      startAt: 'Start time', endAt: 'End time', matchedKeyword: 'Matched keyword', deleteSelected: 'Delete selected ({count})', deleteByFilter: 'Delete by filter',
    ...
    messages: { saved: 'Prompt Audit configuration saved; plaintext API Key state was cleared.', probeSucceeded: 'The audit node is reachable.', deleted: 'Deleted {count} audit events.', flaggedHashDeleted: 'Blocked hash deleted.', flaggedHashNotFound: 'That blocked hash was not found.', flaggedHashesCleared: 'Cleared {count} blocked hashes.' },
    errors: {
      ...
      prompt_guard_requires_audit_enabled: 'Enable Prompt Audit before synchronous blocking.', prompt_audit_invalid_endpoint: 'The audit node configuration is invalid.', prompt_audit_endpoint_required: 'Enable at least one audit node before enabling Prompt Audit.', prompt_audit_groups_required: 'Select at least one group in selected-group mode.', prompt_audit_scanners_required: 'Enable at least one risk category.', prompt_audit_invalid_keyword_mode: 'The keyword blocking strategy is invalid.',
      prompt_audit_invalid_proxy: 'The proxy server does not exist, is inactive, or has expired.', prompt_audit_proxy_unavailable: 'The Prompt Audit proxy service is unavailable.',
      flaggedHashDeleteFailed: 'Unable to delete the blocked hash.', flaggedHashesClearFailed: 'Unable to clear blocked hashes.',
      PROMPT_AUDIT_HASH_INVALID: 'The prompt hash must be a valid 64-character SHA-256 digest.', PROMPT_AUDIT_HASH_CACHE_UNAVAILABLE: 'The prompt-audit hash cache is temporarily unavailable.',
    },
  },
}
