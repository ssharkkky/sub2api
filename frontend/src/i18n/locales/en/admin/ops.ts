export default {
    ops: {
      ...
      requestErrors: 'Service Failures',
      ...
      notApplicable: 'No samples',
      platformFailures: 'Platform Failures',
      providerFailures: 'Provider Failures',
      failureCount: 'Failures',
      unknownFailures: 'Unclassified failures',
      ignoredUserEvents: 'Ignored user-side events',
      recoveredRequests: 'Recovered requests',
      businessLimited: 'Business limits',
      errors: 'Errors',
      ...
      healthBreakdown: {
        ...
        totalDeduction: '-{points} points',
        noDeduction: 'No points deducted',
        infrastructureOnly: 'No business traffic in this window. The score currently reflects infrastructure health only.',
        componentScore: 'Component score {score}/100 · Final weight {weight}%',
        allHealthy: 'No component caused a score deduction.',
        footer: 'The breakdown and score share one backend calculation; optional jobs disabled by an administrator are not penalized.',
        unavailable: 'Score details are unavailable for this sample. Refresh after the backend upgrade completes.',
        durationSeconds: '{value}s',
        durationMinutes: '{value}min',
        durationHours: '{value}h',
        components: {
          business_quality: 'Request quality',
          business_latency: 'First-token latency',
          infrastructure_storage: 'Database and Redis',
          infrastructure_compute: 'CPU and memory',
          infrastructure_jobs: 'Background jobs',
          unknown: 'Other component'
        },
        reasons: {
          databaseUnavailable: 'Database health check failed.',
          redisUnavailable: 'Redis health check failed.',
          cpuHigh: 'CPU usage is {value}%, above the no-deduction threshold of {threshold}%.',
          memoryHigh: 'Memory usage is {value}%, above the no-deduction threshold of {threshold}%.',
          requestErrorRateHigh: 'Request error rate is {value}%, above the no-deduction threshold of {threshold}%.',
          upstreamErrorRateHigh: 'Upstream error rate is {value}%, above the no-deduction threshold of {threshold}%.',
          ttftHigh: 'P99 time to first token is {value}ms, above the no-deduction threshold of {threshold}ms.',
          jobFailed: 'The latest run of background job {job} failed.',
          jobStale: 'Background job {job} has not succeeded for {age}; its allowed interval is {limit}.',
          unknown: 'The backend reported an unrecognized deduction reason.'
        }
      },
      diagnosis: {
      ...
      errorLog: {
        ...
        outcomes: {
          ...
          clientIgnored: 'Client issue · ignored',
          businessIgnored: 'Business limit · ignored',
          securityIgnored: 'Security block · ignored',
          cancelledIgnored: 'Client cancelled · ignored',
      ...
      requestDetails: {
        ...
        sort: {
          label: 'Sort',
          newest: 'Newest requests',
          ttftDesc: 'TTFT longest first',
          ttftAsc: 'TTFT shortest first',
          durationDesc: 'Total duration longest first'
        },
        kind: {
          success: 'SUCCESS',
          error: 'ERROR'
      ...
      alertEvents: {
        title: 'Alert Events',
        description: 'Recent firing/recovery events with the durable email queue’s actual delivery state.',
        loading: 'Loading...',
      ...
      alertRules: {
		...
		health: {
		  evaluatorCoverage: 'Evaluation coverage',
        ...
        metrics: {
		  ...
		  availabilityFailureRate: 'Infrastructure Availability Failure Rate (%)',
		  platformFailureRate: 'Platform-attributed Failure Rate (%)',
		  providerFailureRate: 'Provider-attributed Failure Rate (%)',
		  unknownFailureRate: 'Unknown-attribution Failure Rate (%)',
		  platformCapacityFailureCount: 'Platform Capacity Failures',
		  platformCredentialFailureCount: 'Managed Credential Failures',
		  compatibilityErrorCount: 'Product Compatibility Errors',
		  clientRejectedCount: 'Client Rejections',
		  businessLimitedCount: 'Business Limits',
		  cancelledCount: 'Client Cancellations',
		  securityBlockedCount: 'Security Rejections',
		  recoveredProviderErrorCount: 'Recovered Provider Errors',
		  ttftP95: 'TTFT P95 (seconds)',
		  ttftP99: 'TTFT P99 (seconds)',
		  ttftMax: 'TTFT Max (seconds)',
          p95: 'P95 Latency (ms)',
          p99: 'P99 Latency (ms)',
          cpu: 'CPU Usage (%)',
          memory: 'Memory Usage (%)',
        ...
        metricDescriptions: {
		  ...
		  availabilityFailureRate: 'Final platform, provider, and unknown-attribution failures among SLA-eligible requests.',
		  platformFailureRate: 'Final platform-attributed failures among SLA-eligible requests.',
		  providerFailureRate: 'Final provider-attributed failures among SLA-eligible requests.',
		  unknownFailureRate: 'Unclassified final failures conservatively included in SLA.',
		  platformCapacityFailureCount: 'Final failures caused by routing capacity, account concurrency, or queue capacity.',
		  platformCredentialFailureCount: 'Failures caused by managed account tokens, OAuth credentials, or account authentication.',
		  compatibilityErrorCount: 'Protocol mapping, status rewriting, or client compatibility errors; use P2/P3 only.',
		  clientRejectedCount: 'Invalid requests, authentication failures, or unsupported models excluded from infrastructure SLA.',
		  businessLimitedCount: 'User balance, plan, quota, or personal concurrency limits excluded from infrastructure SLA.',
		  cancelledCount: 'Client cancellations or disconnects, excluded from infrastructure paging by default.',
		  securityBlockedCount: 'Security, risk-control, or content-policy rejections for the separate security channel.',
		  recoveredProviderErrorCount: 'Requests that succeeded after upstream rate limits, 5xx responses, or failover.',
		  ttftP95: 'P95 time to first token among requests with TTFT observations in the window, in seconds.',
		  ttftP99: 'P99 time to first token among requests with TTFT observations in the window, in seconds.',
		  ttftMax: 'Maximum time to first token in the window, in seconds; use a minimum sample gate to reduce low-traffic noise.',
        ...
        table: {
		  ...
		  severity: 'Severity',
		  enabled: 'Enabled',
		  evaluation: 'Evaluation / bad samples',
		  actions: 'Actions'
        },
        form: {
          name: 'Name',
          ...
          advanced: 'Advanced evaluation and recovery',
          advancedHint: 'Defaults suit most cases. Sample gates, recovery hysteresis, and incident families reduce low-traffic false positives and duplicate alerts.',
		  cooldown: 'Cooldown (minutes)',
		  minimumSamples: 'Minimum samples',
		  minimumBadCount: 'Minimum bad samples',
		  recoveryOperator: 'Recovery operator',
		  recoveryThreshold: 'Recovery threshold',
		  recoverySustained: 'Recovery duration (minutes)',
		  incidentFamily: 'Incident family',
		  enabled: 'Enabled',
		  notifyEmail: 'Send email notifications',
		  shadowMode: 'Shadow mode (record without creating alerts)'
        },
        validation: {
		  ...
		  windowRange: 'Window must be one of: 1, 5, 15, 30, or 60 minutes',
          sustainedRange: 'Sustained must be between 1 and 1440 samples',
		  cooldownRange: 'Cooldown must be between 0 and 1440 minutes',
		  minimumSamplesRange: 'Minimum samples must be a non-negative integer',
		  minimumBadCountRange: 'Minimum bad samples must be a non-negative integer',
		  incidentFamily: 'Incident family may contain only lowercase letters, numbers, underscores, or hyphens',
		  recoveryPair: 'Recovery operator and threshold must be configured together',
		  recoverySustainedRange: 'Recovery duration must be between 1 and 1440 minutes'
        }
      },
      runtime: {
      ...
      email: {
        ...
        recipientsHint: 'No email is sent when recipients are empty; the system does not silently fall back to another administrator.',
        ...
        validation: {
          ...
          rateLimitRange: 'Rate limit per hour must be between 0 and 10000',
      ...
      settings: {
        ...
        alertConfig: 'Alert Notification Behavior',
        emailPolicyOwnershipHint: 'Manage channel-level switches and recipients in System Settings → Email. This page only controls Ops alert behavior.',
        ...
        minSeverityHint: 'Only events at or above this level are sent. P0 maps to critical, P1 to warning, and other severities to info.',
        rateLimitHint: 'Maximum Ops alert emails per recipient per hour. Set 0 for unlimited; excess events remain visible in Ops.',
        batchWindowHint: 'Repeated transitions in the same incident family send only the first email during this window. Set 0 to disable.',
        includeResolvedHint: 'Send one recovery message after the incident resolves so on-call staff can close the loop.',
        reportConfig: 'Ops Report Schedules',
        reportBehaviorHint: 'Schedules send only when the global Ops report channel is enabled and has valid recipients. Empty groups fail visibly and never fall back to another administrator.',
        serverTimezone: 'Execution timezone: {timezone}. All send times use the server timezone.',
        sendTime: 'Send time',
        legacyScheduleHint: 'A legacy custom schedule is preserved. Selecting a new time converts it to the standard schedule shown here.',
        weekday: 'Day of week',
        nextRun: 'Estimated next run: {time}',
        nextRunAfterSave: 'calculated after saving',
        weekdays: {
          monday: 'Monday',
          tuesday: 'Tuesday',
          wednesday: 'Wednesday',
        ...
        requestErrorRateMaxPercent: 'Service Failure Rate Maximum (%)',
        requestErrorRateMaxPercentHint: 'Only platform, provider, and unclassified failures count. Client issues, business limits, security blocks, and cancellations are ignored.',
      ...
      realtime: {
        title: 'Realtime',
		actualDeduction: 'Actual charged',
		windowTotal: '{window} real data',
		noChartData: 'No data in this window',
      ...
      tooltips: {
        ...
        throughputTrend: 'Request RPM and K TPS trends in the selected window.',
        ...
        qps: 'Requests Per Minute (RPM) and thousands of Tokens Per Second (K TPS), shown in real time.',
        tokens: 'Total number of tokens processed in the current time window.',
        sla: 'Platform availability = successes / (successes + platform failures + provider failures + unclassified failures). Client input, business limits, security blocks, and cancellations are excluded; empty windows show “No samples”.',
        errors: 'Platform failures contain only platform-owned faults. Client issues, business limits, security blocks, and cancellations are classified separately and ignored.',
        upstreamErrors: 'Provider failures include only final provider-owned failures. Requests recovered by retry or account failover are shown separately.',
