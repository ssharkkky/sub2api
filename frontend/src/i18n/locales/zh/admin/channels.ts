export default {
    ...
    channels: {
      ...
      form: {
        ...
        tierMultiplierHint: '覆盖上方渠道默认档位倍率；留空则用渠道默认，再没有就用模型目录价。',
        maxReasoningEffortMultiplier: 'Max 推理倍率',
        fable51DefaultMaxReasoningMultiplier: '默认 3',        multiplierPlaceholder: '未配置',
        ...
        serviceTiersHint: '控制该渠道是否允许各服务档位，并设置相对 Standard 基准价的默认计费倍率。模型定价里的 Fast/Flex 会覆盖这里。',
        serviceTierStandard: 'Standard',
        serviceTierStandardHint: '普通请求以及未指定 service_tier 的请求。',
        serviceTierPriority: 'Fast / Priority',
        serviceTierPriorityHint: '客户端 fast 会统一转换为 priority 后发送上游。',
        serviceTierFlex: 'Flex',
        serviceTierFlexHint: '是否可用仍取决于最终上游模型和账号。',
        serviceTierMultiplier: '{tier} 计费倍率',
        serviceTierConfigInvalid: '当前保存的服务档位配置无效，请修正后重新保存。',
    ...
    channelMonitor: {
      ...
      form: {
        ...
        apiKeyProviderChangeRequired: '切换平台后必须提供新的 API Key',
        apiKeyProviderChangeHint: '外部上游可直接填写新 Key；使用本站 Antigravity 分组时请点“使用我的 Key”，系统会自动匹配专用路由。',
        useMyKey: '使用我的 Key',
