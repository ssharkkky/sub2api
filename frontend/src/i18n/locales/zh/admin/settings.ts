export default {
    settings: {
      ...
      payment: {
        ...
        refundResultUserEmailHint: '退款成功、最终失败或管理员拒绝退款申请后通知用户；邮件设置中的“用户退款通知”总开关也必须开启',
        easypayAutoReconcile: 'EasyPay 主动查单',
        easypayAutoReconcileHint: '每分钟主动查询待支付的 EasyPay 订单。异步回调丢失时可自动补到账；关闭后仍保留异步回调和用户回跳查单',
      ...
      emailPolicy: {
        ...
        recipientGroupsHint: '这些邮箱用于接收账号配额、退款审核和运维通知。只展示当前有邮件通道使用的收件组。',
        userPrimary: '发送至账户邮箱',
        userPrimaryHint: '用户注册或登录账号所使用的主邮箱。',
        verifiedAdditional: '同时发送至已验证的备用通知邮箱',
        verifiedAdditionalHint: '用户在个人资料中额外添加、已经验证且未禁用的通知邮箱。',
        userAccountEmailRecipient: '发送至事件关联用户的账户邮箱',
        eventRecipient: '由当前业务事件指定收件人',
        groupRecipientSummary: '发送到选定的管理员收件人组',
        balanceRecipientSummary: '发送至账户邮箱和/或用户已验证的备用通知邮箱',
        userPrimaryRecipientSummary: '发送至事件关联用户的账户邮箱',
        explicitRecipientSummary: '验证码或链接只发送到当前操作指定的邮箱',
        addRecipient: '添加收件人',
        addToAllGroups: '添加到全部收件组',
        bulkEmailPlaceholder: "输入一次管理员邮箱，例如 admin{'@'}example.com",
        addedToAllGroups: '已添加到全部管理员收件组，保存后生效',
        invalidEmail: '请输入合法的管理员邮箱地址',
        recipientGroupFull: '“{group}”最多只能配置 20 个收件人',
        balanceRecipientRequired: '启用余额通知时，至少需要选择账户邮箱或备用通知邮箱之一',
        ...
        balanceLow: {
          title: '低余额提醒',
          hint: '余额从阈值上方降到阈值下方时发送一次提醒；充值成功通知仍由余额通知总开关控制。',
          threshold: '提醒阈值（USD）',
          thresholdRequired: '启用低余额提醒时，提醒阈值必须大于 0',
          rechargeURL: '充值页面地址（可选）',
          invalidRechargeURL: '充值页面地址必须是完整的 HTTP 或 HTTPS 地址'
        },
        channels: {
