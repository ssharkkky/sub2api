...
func TestOpenAIFastBilling_FastMultiplierOverridesEnforcedRatio(t *testing.T) {
	...
	standard := svc.computeTokenBreakdown(pricing, tokens, 1, "", false, nil)
	fast := svc.computeTokenBreakdown(pricing, tokens, 1, "fast", false, nil)
	priority := svc.computeTokenBreakdown(pricing, tokens, 1, "priority", false, nil)
	...
	enforced := svc.computeTokenBreakdown(&withoutOverride, tokens, 1, "fast", false, nil)
