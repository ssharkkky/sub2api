...
func (a *Account) IsModelSupported(requestedModel string) bool {
	...
	if len(mapping) > 0 {
		...
		// rename-only 映射（model_mapping_restricts 显式为 false）仅做改名，不构成
		// 白名单：未映射模型沿用上游快照兜底判定；其余显式映射即显式策展，未覆盖
		// 模型不得经"无快照放行"兜底漏出（管理员 allowlist 保持受限，对齐上游语义）。
		if restricts, ok := a.Credentials[CredentialKeyModelMappingRestricts].(bool); !ok || restricts {
			return false
		}
