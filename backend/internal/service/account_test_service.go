...
func (s *AccountTestService) FetchOpenAIAccountModels(ctx context.Context, account *Account) ([]openai.Model, error) {
	...
	if account != nil && account.IsOpenAIOAuthLike() {
		...
		for _, model := range openai.DefaultModels {
			if IsGPTImageGenerationModel(model.ID) && openAIImagesPickerModelSupported(account, model.ID) && !seen[model.ID] {
...
// openAIImagesPickerModelSupported 判定 OAuth 测试选择器是否注入本地支持的图像模型。
// 显式 model_mapping 账号按映射严格判定（映射 = 显式策展，未列出的模型不注入）；
// 无映射账号沿用 IsModelSupported（含上游快照兜底）。
func openAIImagesPickerModelSupported(account *Account, modelID string) bool {
	if mapping := account.GetModelMapping(); len(mapping) > 0 {
		return mappingSupportsRequestedModel(mapping, modelID)
	}
