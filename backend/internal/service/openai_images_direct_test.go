...
func TestCodexDirectImagesMultipleOutputs(t *testing.T) {
	t.Run("non_streaming_fans_out", func(t *testing.T) {
		body := []byte(`{"model":"gpt-image-2.5-flare","prompt":"draw","n":2}`)
		c, _ := newOpenAIImagesTestContext(t, body)
		upstream := &httpUpstreamRecorder{responses: []*http.Response{
			{StatusCode: 200, Header: http.Header{"Content-Type": {"text/event-stream"}}, Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.completed\",\"response\":{\"usage\":{\"input_tokens\":10,\"output_tokens\":20,\"output_tokens_details\":{\"image_tokens\":20}},\"tool_usage\":{\"image_gen\":{\"images\":1}},\"output\":[{\"type\":\"image_generation_call\",\"result\":\"AA==\"}]}}\n\ndata: [DONE]\n\n",
			))},
			{StatusCode: 200, Header: http.Header{"Content-Type": {"text/event-stream"}}, Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.completed\",\"response\":{\"usage\":{\"input_tokens\":10,\"output_tokens\":20,\"output_tokens_details\":{\"image_tokens\":20}},\"tool_usage\":{\"image_gen\":{\"images\":1}},\"output\":[{\"type\":\"image_generation_call\",\"result\":\"AQ==\"}]}}\n\ndata: [DONE]\n\n",
			))},
		}}
		svc := newOpenAIImagesTestService(upstream)
		parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	...
	t.Run("streaming_rejects_multi", func(t *testing.T) {
		...
		parsed, err := svc.ParseOpenAIImagesRequest(c, body)
		require.NoError(t, err)
		// 上游契约：OAuth 图像流式单次仅 1 张，多图必须走非流式扇出
...
func TestCodexDirectImagesStreaming(t *testing.T) {
	for _, test := range []struct {
		...
		{"partial_requested_multiple", "complete", 0, true, false}, // 上游契约：流式 n>1 前置拒绝
