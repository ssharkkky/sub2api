...
func buildOpenAIImagesOAuthPayload(parsed *OpenAIImagesRequest, model string) ([]byte, string, error) {
	...
	// 透传客户端请求的 service tier（fast/priority 策略已在 ForwardImages
	// 统一改写 parsed.ServiceTier）；direct 协议与 Responses 协议口径一致。
	if tier := strings.TrimSpace(parsed.ServiceTier); tier != "" {
