...
func TestConfiguredServiceTierMultiplier(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.InDelta(t, tt.want, configuredServiceTierMultiplier(tt.serviceTier, tt.pricing, nil), 1e-12)
		})
	}
}

func TestConfiguredServiceTierMultiplierUsesSingleResolutionChain(t *testing.T) {
	...
	require.InDelta(t, 3, configuredServiceTierMultiplier("fast", &ModelPricing{FastMultiplier: pricingMultiplier(3)}, snapshot), 1e-12)
	require.InDelta(t, 2, configuredServiceTierMultiplier("fast", &ModelPricing{}, snapshot), 1e-12)
	require.InDelta(t, 0.25, configuredServiceTierMultiplier("flex", &ModelPricing{FlexMultiplier: pricingMultiplier(0.25)}, snapshot), 1e-12)
	require.InDelta(t, 0.4, configuredServiceTierMultiplier("flex", &ModelPricing{}, snapshot), 1e-12)
	require.InDelta(t, 1.25, configuredServiceTierMultiplier("", &ModelPricing{}, snapshot), 1e-12)
	require.InDelta(t, 2, configuredServiceTierMultiplier("fast", &ModelPricing{}, nil), 1e-12)
}
...
func TestConfiguredServiceTierMultiplierAppliesToEveryTokenComponent(t *testing.T) {
	...
	fast := service.computeTokenBreakdown(pricing, tokens, 1, "fast", false, nil)
	...
	flex := service.computeTokenBreakdown(pricing, tokens, 1, "flex", false, nil)
