...
func TestListGroups_ChannelPricingWinsOverGroupTokenCard(t *testing.T) {
	...
	require.NoError(t, err)
	m := out[0].Models[0]
	require.InDelta(t, 3e-6, *m.Pricing.InputPrice, 1e-15, "渠道平价优先于分组价卡")
	require.InDelta(t, 1.5e-5, *m.Pricing.OutputPrice, 1e-15, "渠道配置的 output 价保留")
