...
func TestGetModelPricingWithChannel_OverrideInputPriceOnly(t *testing.T) {
	...
	// Channel prices are Standard base prices. Priority stays unset so the
	// request-scoped channel tier multiplier is the only tier adjustment.
...
func TestGetModelPricingWithChannel_CacheReadPriceDoesNotSynthesizePriority(t *testing.T) {
	...
	// Priority is not a second copy of Standard; the channel tier multiplier
	// is applied later from the immutable request snapshot.
