...
func TestComputeTokenBreakdown_GptImage2ImageEditIssue4386(t *testing.T) {
	...
	cost := svc.computeTokenBreakdown(pricing, tokens, 1.0, "", false, nil)
...
func TestGetModelPricingWithChannel_OverrideInputPriceOnly(t *testing.T) {
	...
	// Channel override of Standard. claude-sonnet-4 has no catalog priority
	// ratio, so the priority slot stays zero: OpenAI uses the request-scoped
	// channel service-tier snapshot, others use configuredServiceTierMultiplier.
...
func TestGetModelPricingWithChannel_CacheReadPriceDoesNotSynthesizePriority(t *testing.T) {
	svc := newTestBillingService()
	...
	// claude-sonnet-4 has no catalog tier ratio to preserve. OpenAI Fast/Flex
	// still uses the channel service-tier snapshot, not this slot.
...
func TestComputeTokenBreakdown_ExplicitZeroImagePrice_NoFallback(t *testing.T) {
	...
	bd := svc.computeTokenBreakdown(pricing, tokens, 1.0, "", false, nil)
