...
func TestAPIKeyService_RejectsV19AuthSnapshotWithoutGroupModelPricing(t *testing.T) {
	svc := &APIKeyService{}
	...
	if ok {
		t.Fatal("expected v19 auth snapshot to be rejected after group model pricing was added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
