...
func openAICompatibleAccountEligibilityFailureReason(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) string {
	return openAICompatibleAccountEligibilityFailureReasonWithSupportModel(ctx, account, platform, requestedModel, requestedModel, requireCompact, requiredCapability)
}

// openAICompatibleAccountEligibilityFailureReasonWithoutModelSupport leaves the
// account model support check to the caller so OpenAI channel mappings can be
// resolved first (the account sees the mapped model, not the request alias).
func openAICompatibleAccountEligibilityFailureReasonWithoutModelSupport(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) string {
	return openAICompatibleAccountEligibilityFailureReasonWithSupportModel(ctx, account, platform, requestedModel, "", requireCompact, requiredCapability)
}

func openAICompatibleAccountEligibilityFailureReasonWithSupportModel(ctx context.Context, account *Account, platform string, requestedModel string, supportModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) string {
	if reason := openAICompatibleAccountEligibilityFailureReasonBeforeProfit(ctx, account, platform, requestedModel, supportModel, requireCompact, requiredCapability); reason != "" {
		return reason
	}
...
func isOpenAICompatibleAccountEligibleForRequestBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, supportModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	return openAICompatibleAccountEligibilityFailureReasonBeforeProfit(ctx, account, platform, requestedModel, supportModel, requireCompact, requiredCapability) == ""
}

func openAICompatibleAccountEligibilityFailureReasonBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, supportModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) string {
	...
	if !account.IsSchedulableForModelWithContext(ctx, schedulableModel) {
...
func (s *OpenAIGatewayService) isAccountEligibleForRequest(
	ctx context.Context,
	groupID *int64,
	account *Account,
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if !s.isAccountEligibleForRequest(ctx, groupID, account, platform, requestedModel, false, requiredCapability) {
		return nil
	}
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool, openAISelectionFilterStats) {
	...
	for i := range accounts {
		...
		fresh := s.resolveFreshSchedulableOpenAIAccountBeforeProfit(ctx, groupID, acc, platform, requestedModel, false, requiredCapability)
		if fresh == nil {
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	if sessionHash != "" {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky && s.isAccountEligibleForRequest(ctx, groupID, account, platform, requestedModel, false, requiredCapability) {
					account = s.recheckSelectedOpenAIAccountFromDB(ctx, account, groupID, platform, requestedModel, requireCompact, requiredCapability)
					if account == nil {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
	...
	for i := range accounts {
		...
		if reason := openAICompatibleAccountEligibilityFailureReasonWithoutModelSupport(ctx, acc, platform, requestedModel, false, requiredCapability); reason != "" {
			filterStats.exclude(reason)
			continue
		}
		if !s.isAccountModelSupportedForRequest(ctx, groupID, acc, requestedModel) {
	...
	tryAcquireFromLoadMap := func(loadMap map[int64]*AccountLoadInfo) (*AccountSelectionResult, bool, error) {
		...
		for _, item := range selectionOrder {
			fresh := s.resolveFreshSchedulableOpenAIAccount(ctx, groupID, item.account, platform, requestedModel, false, requiredCapability)
			if fresh == nil {
				continue
			}
	...
	if err != nil {
		...
		for _, acc := range ordered {
			fresh := s.resolveFreshSchedulableOpenAIAccount(ctx, groupID, acc, platform, requestedModel, false, requiredCapability)
			if fresh == nil {
				continue
			}
			fresh = s.recheckSelectedOpenAIAccountFromDB(ctx, fresh, groupID, platform, requestedModel, requireCompact, requiredCapability)
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccount(ctx context.Context, groupID *int64, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	fresh := s.resolveFreshSchedulableOpenAIAccountBeforeProfit(ctx, groupID, account, platform, requestedModel, requireCompact, requiredCapability)
	if fresh == nil {
		return nil
	}
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccountBeforeProfit(ctx context.Context, groupID *int64, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	if account == nil {
		return nil
	}
	platform = NormalizeOpenAICompatiblePlatform(platform)
