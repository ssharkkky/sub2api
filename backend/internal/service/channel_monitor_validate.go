...
// monitorProviders 渠道监控支持的全部 provider（与迁移 278 的 11 平台 CHECK 约束一致）。
// 不再以 adapter 表为唯一来源：antigravity、kiro 与 opencode_go 没有探活 adapter，但支持配额模式。
//
...
// opencode_go 走 OpenCode OAuth（Zen/GO）协议，无探活 adapter，只允许配额模式。
//
//nolint:gochecknoglobals // 静态查表，初始化后不变。
...
//   - 显式 /antigravity 路由仅 Anthropic/Gemini 可用（强制 Antigravity 平台）
...
func validateStoredMonitorEndpointRoute(provider, endpoint string) error {
	u, err := url.Parse(strings.TrimSpace(endpoint))
	if err != nil {
		return ErrChannelMonitorInvalidEndpoint
	}
	if strings.HasPrefix(u.Path, "/antigravity") && !isAllowedMonitorEndpointPath(provider, u.Path) {
		return ErrChannelMonitorEndpointPath
	}
	return nil
}
