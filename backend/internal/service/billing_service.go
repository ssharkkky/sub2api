...
const (
	// xAI 官方长上下文价卡：整单输入（含 cache）达到 200K 即切换长上下文单价
	// （输入/输出均 2x，阈值含边界，区别于 GPT 的严格 >272K）。
...
func usePriorityServiceTierPricing(serviceTier string, pricing *ModelPricing, snapshot *ChannelServiceTierSnapshot) bool {
	if pricing == nil || snapshot != nil {
		return false
	}
...
func commercialTierForBilling(serviceTier string) OpenAICommercialServiceTier {
	if tier, ok := NormalizeOpenAICommercialTier(serviceTier); ok {
		return tier
	}
	return OpenAICommercialTierStandard
}

func configuredServiceTierMultiplier(serviceTier string, pricing *ModelPricing, snapshot *ChannelServiceTierSnapshot) float64 {
	commercial := commercialTierForBilling(serviceTier)
	if pricing != nil {
		switch commercial {
	...
	if snapshot != nil {
		if option, ok := snapshot.Config.OptionForTier(commercial); ok {
	...
	if s.pricingService != nil {
		...
		if litellmPricing != nil {
			...
			return s.applyModelSpecificPricingPolicyEx(model, &ModelPricing{
				...
				// 目录显式 long_context_threshold_inclusive 优先；解析阶段已对未显式
				// 给出的提供商按 xAI"达到即进高档"启发式补齐，其余严格大于。
...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	...
	if input.ServiceTierSnapshot == nil && resolved != nil && resolved.ServiceTierSnapshot != nil {
		input.ServiceTierSnapshot = resolved.ServiceTierSnapshot
	}
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	breakdown := s.computeTokenBreakdown(pricing, input.Tokens, input.RateMultiplier, input.ServiceTier, applyLongCtx, input.ServiceTierSnapshot)
...
func (s *BillingService) computeTokenBreakdown(
	...
	if usePriorityServiceTierPricing(serviceTier, pricing, snapshot) {
		...
		tierMultiplier = configuredServiceTierMultiplier(serviceTier, pricing, snapshot)
	}
	...
	if longContextPricingEligible {
		baselineCost = s.computeTokenBreakdown(pricing, tokens, rateMultiplier, serviceTier, false, snapshot)
...
func (s *BillingService) calculatePerRequestCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	tierMultiplier := configuredServiceTierMultiplier(input.ServiceTier, resolved.BasePricing, input.ServiceTierSnapshot)
	if tierMultiplier != 1 {
		totalCost *= tierMultiplier
	}
...
func (s *BillingService) calculateCostInternalWithPolicy(
	...
	return s.computeTokenBreakdown(pricing, tokens, rateMultiplier, serviceTier, longContextBillingEnabled, nil), nil
}
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	// xAI Grok：目录 above_Xk / 渠道价卡未带长上下文档的 Grok 型号补齐官方 ≥200K 档；
	// 非 Grok 型号与档位完整的型号原样返回（见 applyGrokLongContextPolicy 内部守卫）。
	pricing = s.applyGrokLongContextPolicy(model, pricing)
