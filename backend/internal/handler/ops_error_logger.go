...
func OpsErrorLoggerMiddleware(ops *service.OpsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var clientIP string
		if ip := strings.TrimSpace(ip.ExactClientIP(c)); ip != "" {
			clientIP = ip
...
func logOpsRecoveredUpstream(c *gin.Context, ops *service.OpsService, finalStatus int) {
	...
	// fork: recovered rows keep their own final-outcome classification and stay
	// outside the failed-request SLA.
...
func classifyOpsPhase(errType, message, code string) string {
	...
	case "rate_limit_error":
		...
		return "upstream"
	case "invalid_request_error", "image_generation_user_error", "permission_error", "forbidden_error", "not_found_error", "model_not_found":
		return "request"
...
func classifyOpsSeverity(errType string, status int) string {
	switch errType {
	case "invalid_request_error", "image_generation_user_error", "authentication_error", "permission_error", "forbidden_error", "not_found_error", "model_not_found", "billing_error", "subscription_error":
		return "P3"
	}
	if status >= 500 {
		return "P1"
