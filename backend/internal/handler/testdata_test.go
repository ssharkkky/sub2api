...
// 加载（镜像种子/缓存）或远程合并文档。gateway 的 /v1/models 等接口经
// PricingService 间接消费全局目录，测试用同一份仓库权威数据
// （deploy/data/models.json 的 models 段）经生产解析路径播种全局目录，
// 使依赖「真实在售目录」的 handler 测试与生产数据保持同源。
...
func seedTestCatalog() error {
	...
	cat, err := modelcatalog.Load(section)
	if err != nil {
