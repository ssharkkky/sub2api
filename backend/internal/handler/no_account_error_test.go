...
func TestClassifyNoAccountError_NilDiagnoser_Falls503(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(c, nil, apiKey, "gpt-5", "gpt-5", service.PlatformOpenAI, nil)
...
func TestClassifyNoAccountError_NilAPIKey_Falls503(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(c, fd, nil, "gpt-5", "gpt-5", service.PlatformOpenAI, nil)
...
func TestClassifyNoAccountError_NilGroupID_Falls503(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(c, fd, apiKey, "gpt-5", "gpt-5", service.PlatformOpenAI, nil)
...
func TestClassifyNoAccountError_EmptyModel_Falls503(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(c, fd, apiKey, "   ", "", service.PlatformOpenAI, nil)
...
func TestClassifyNoAccountError_ModelNotSupported_Returns404(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(c, fd, apiKey, "gpt-5.1-codex-mini", "gpt-5.1-codex-mini", service.PlatformOpenAI, nil)
...
func TestClassifyNoAccountError_ChannelModelRestricted_Returns404WithoutDiagnosis(t *testing.T) {
	c := newTestGinContextWithRequest()
	fd := &fakeDiagnoser{resp: service.ModelAvailabilityDiagnosis{HasAccountsInPool: true, HasModelSupport: true}}
	apiKey := &service.APIKey{GroupID: ptrInt64(42)}
	cause := fmt.Errorf("selection failed: %w", service.ErrChannelModelRestricted)

	cls := classifyNoAccountErrorFromGin(c, fd, apiKey, "gpt-5.6-luna", "gpt-5.6-luna", service.PlatformOpenAI, cause)

	require.Equal(t, http.StatusNotFound, cls.Status)
	require.Equal(t, "model_not_found", cls.ErrType)
	require.True(t, cls.ModelNotFound)
	require.Equal(t, `Model "gpt-5.6-luna" is not available for this group`, cls.Message)
	require.Empty(t, fd.calls, "typed channel restriction must not be reclassified as temporary capacity")
}
...
func TestClassifyOpenAICompatibleNoAccountError_GrokUsesGrokPlatform(t *testing.T) {
	...
	cls := classifyOpenAICompatibleNoAccountErrorFromGin(c, fd, apiKey, "grok-4.5", "grok-4.5", nil)
...
func TestClassifyNoAccountError_PureClassifierDoesNotMarkGinContext(t *testing.T) {
	...
	cls := classifyNoAccountError(c.Request.Context(), fd, apiKey, "gpt-5", "gpt-5", service.PlatformOpenAI, nil)
...
func TestClassifyNoAccountError_ModelSupportedOnlyByRateLimitedAccount_Returns503(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(c, fd, apiKey, "claude-opus-4-8", "claude-opus-4-8", service.PlatformAnthropic, nil)
...
func TestClassifyNoAccountError_DisplayModelOverridesRoutingForMessage(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(c, fd, apiKey, "gpt-5", "claude-3-fancy", service.PlatformOpenAI, nil)
...
func TestClassifyNoAccountError_FromGin_NilContextStillSafe(t *testing.T) {
	...
	cls := classifyNoAccountErrorFromGin(nil, fd, apiKey, "gpt-5", "gpt-5", service.PlatformOpenAI, nil)
