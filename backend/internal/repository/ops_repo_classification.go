...
SELECT
  ...
  COUNT(*) FILTER (WHERE error_category = 'platform_credential' AND COALESCE(counts_toward_sla, NOT COALESCE(is_business_limited, false))),
  COUNT(*) FILTER (WHERE alert_family = 'compatibility'),
