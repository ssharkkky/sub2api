...
const groupModelAllowlistRepairMigration = "276_group_model_allowlist_repair.sql"

// 276（upstream 236 重编号）是可重放的修复迁移：fork 275 采用 expand/contract
// （新增可空列 + 触发器回填 + 旧列保留）。数据库若回到旧结构（手工删掉新列与
// 触发器、按旧结构部分恢复）应用仍能启动，但所有关联 groups 的查询都会报
// column groups.model_allowlist does not exist（issue #6780）。276 不含裸 RENAME：
// 旧列保留，回填 UPDATE 复制旧列配置，最终收敛为 NOT NULL DEFAULT '{}'::jsonb。
func TestMigration276RepairsLegacyModelsListConfigColumn(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()

	// 模拟回滚到旧结构：新列与其触发器被手工移除，旧列保留。
	_, err := tx.ExecContext(ctx, "DROP TRIGGER IF EXISTS groups_default_model_allowlist ON groups")
...
VALUES ('migration-276-repair', 'anthropic', 1, 'active', '{"enabled":true,"models":["claude-sonnet-5"]}'::jsonb)
RETURNING id
`).Scan(&groupID))

	applyGroupModelAllowlistRepair(ctx, t, tx)

	// 旧列配置经回填保留在新列，且新列恢复 NOT NULL DEFAULT '{}' 的形状。
...
func TestMigration276BackfillsWhenBothColumnsExist(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()

	// fork 275 保留旧列：两列并存即基线状态，无需再建列。

	// 新列仍是默认空值：旧列里的配置应该被补回来。
	var staleID int64
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO groups (name, platform, rate_multiplier, status, model_allowlist, models_list_config)
VALUES ('migration-276-backfill', 'anthropic', 1, 'active', '{}'::jsonb, '{"enabled":true,"models":["legacy-model"]}'::jsonb)
RETURNING id
`).Scan(&staleID))
	...
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO groups (name, platform, rate_multiplier, status, model_allowlist, models_list_config)
VALUES ('migration-276-keep', 'anthropic', 1, 'active', '{"enabled":true,"models":["current-model"]}'::jsonb, '{"enabled":true,"models":["legacy-model"]}'::jsonb)
RETURNING id
`).Scan(&currentID))
...
func TestMigration276RecreatesMissingModelAllowlistColumn(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()
...
VALUES ('migration-276-recreate', 'anthropic', 1, 'active')
RETURNING id
`).Scan(&groupID))
