-- The v20 API-key auth snapshot includes both group pricing fields introduced
-- by migration 234. Extend the durable invalidation trigger so direct SQL
-- updates, or a process crash after commit but before application-level cache
-- invalidation, cannot leave stale pricing in Redis/L1.
--
-- Keep the complete function body here instead of changing historical
-- migration 202. This remains compatible with the previous application: the
-- function is only invoked by the existing AFTER UPDATE/DELETE trigger and
-- preserves its enqueue behavior.
