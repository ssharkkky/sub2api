...
--
-- （fork 重编号：upstream 238 → 278；重建列表补上 fork 独有的 kiro，
-- 与 256/257/258 的约束终态对齐，避免回退 fork 平台。）
--
-- 形态说明（docs/DOCKER_MANAGED_UPDATES.md 托管蓝绿 + 镜像回滚策略）：
-- 与 224/226/227/240/256-258/277 同型——DROP IF EXISTS 后重建超集约束，存量行
-- 瞬时校验通过；DROP 语句统一采用 `ALTER TABLE IF EXISTS ONLY <表>` 前缀，与基线
-- 内已发布的 157/240/245/256-258 语句指纹保持区分。upstream 原版的条件重建 DO 块
-- 收敛为无条件超集重建（可达状态下语义等价，且为 checker 允许形态）。
-- OpenCode GO 走 API key 多协议形态，渠道监控 provider 白名单与 minimax 同档。
