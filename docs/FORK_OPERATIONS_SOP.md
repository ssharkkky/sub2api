...
Release PR 合并且上述 tag 前门禁全部通过后，禁止从本地人工创建或推送发布 tag。仓库 tag ruleset 必须禁止 `v*-ts.*` 的人工创建、更新和删除，唯一 bypass actor 类型是 Deploy Key。仓库只允许保留一把可写 Deploy Key，名称为 `Sub2API release tag promoter`；私钥只保存在 `RELEASE_TAG_DEPLOY_KEY` Actions secret，普通 `GITHUB_TOKEN` 不具备创建发布 tag 的权限。GitHub ruleset API 会隐藏 Deploy Key actor ID，因此最终审计必须同时核对 ruleset 和 repository deploy key 列表。
